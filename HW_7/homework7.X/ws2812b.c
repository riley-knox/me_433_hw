...
void ws2812b_setup() {
    ...
    // initialize pin B6 as output, initially off
...
void ws2812b_setColor(wsColor * c, int numLEDs) {
    ...
    for (i = 0; i < numLEDs; i++) {
        // loop through each color bit, starting with MSB
        // red
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if ((c[i].r >> j) & 1) {        // bitshift right, AND with 1 to get bit value
                // the high is longer
    ...
    for (i = 1; i < numBits; i++) {
        while (TMR2 < delay_times[i]) {;}       // wait for number of specified clock cycles
