 ...
 */
    ...
    for (a = 0; a < 5; a++) {                               // iterate through columns
        unsigned char byte = ASCII[character - 0x20][a];    // column byte to write
        
        for (b = 0; b < 8; b++) {                           // iterate through rows
            unsigned char pix = (byte >> b) & 1;            // get each pixel value by bitshifting right, ANDing with 1
 ...
 */
    ...
    for (n = 0; n < strlen(message); n++) {             // iterate through string
        if ((pos_x + c)*5 > 122) {                                // check to make sure 
            pos_y++;
            c = 0;
        }
